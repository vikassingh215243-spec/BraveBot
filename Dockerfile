...
# 3. Install ALL required Selenium dependencies (CRITICAL FIX for NoClassDefFoundError)
RUN mkdir -p /app/lib && \
...
WORKDIR /app
# COPY . /app copies all local files, including BraveScreenshotBot.jar
COPY . /app

# 6. Command to run the application (Updated Classpath to include all JARs)
